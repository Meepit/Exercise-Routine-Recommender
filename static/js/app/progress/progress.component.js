...
angular.module('progressView').component('progressView', {
    ...
    .controller("progressController", function($scope, $cookies, $http, UserData) {
        ...
        if (loggedIn == null) {
            ...
            $scope.data = [
                []
            ] // Data the graph will be generated from
            ...
            var genGraph = function() {
                // Setup scope variables for angularcharts
                // labels will currently hold integers representing individual entried. Ideally holds dates
                // however dates take up too much screen realestate on vertical mobile devices.
                // Reset graph data
                $scope.data = [
                    []
                ]
                ...
                // Sort data by date
                $scope.exerciseData = UserData.userData(token).get({
                    "username": username
                }, function(data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if (data[i].exercise_id == $scope.selectedExercise[0]) {
                            ...
                            $scope.labels.push(i) //data[i].date)
                        }
                    }
                })
            }
            ...
            $scope.postProgress = function() {
                // Post progress and generate appropriate user feedback messages
                $scope.postSuccMsg = ""
                $scope.postErrMsg = ""
            ...
            $scope.deleteProgress = function() {
                // Delete given progress and generate appropriate user feedback messages
